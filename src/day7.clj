...
(defn parse-fs [cmds]
  (loop [fs []
         ...
         [[cmd & data] & cmds] cmds]
    (cond
      (= 0 (count cmds)) (partition 2 fs)
      (= cmd :cd) (let [pos (update-pos pos data)] (recur fs pos cmds))
      (= cmd :ls) (let [fs (update-fs fs pos data)] (recur fs pos cmds)))))
